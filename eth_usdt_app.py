...
WHATSAPP_TO = "whatsapp:+918309641852"            # your verified WhatsApp number
...
st.caption("Realtime refresh every 1 s + WhatsApp alerts")
...
def send_whatsapp_alert(price):
    try:
        ...
        msg = f"🚨 ETH/USDT Alert → Current Price = {price:.2f} USDT (between {low}-{high})"
...
last_alert = None  # prevent spamming
...
while True:
    ...
    if price:
        ...
        # Alert check (only once per entry into range)
        if low <= price <= high and last_alert != "inside":
